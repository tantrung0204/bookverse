            ...
            <%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
                                        ...
                                        <ul class="pagination">
                                            <%-- Nút Previous --%>
                                                <c:if test="${currentPage > 1}">
                                                    <a class="page-btn"
                                                        href="${pageContext.request.contextPath}/product?tab=${currentTab}&keyword=${currentKeyword}&page=${currentPage - 1}">&laquo;</a>
                                                </c:if>
                                                ...
                                                <%-- Nếu tổng số trang <=maxNode, hiển thị tất cả --%>
                                                    ...
                                                    <%-- Nút Next --%>
                                                        <c:if test="${currentPage < totalPages}">
                                                            <a class="page-btn"
                                                                href="${pageContext.request.contextPath}/product?tab=${currentTab}&keyword=${currentKeyword}&page=${currentPage + 1}">&raquo;</a>
                                                        </c:if>
                                        </ul>
                                    </nav>
                            </div>
                ...
                <script>
                    ...
                    function toggleProductType(modalId, type) {
                        ...
                        if (type === 'Book') {
                            ...
                            if(bookCat) bookCat.style.display = 'block';
                            if(statCat) statCat.style.display = 'none';
                        } else {
                            if(bookFields) bookFields.style.display = 'none';
                            if(statFields) statFields.style.display = 'block';
                            if(bookCat) bookCat.style.display = 'none';
                            if(statCat) statCat.style.display = 'block';
                        }
                    }
                </script>
                ...
                <div id="addPopup" class="modal-overlay" style="display: ${openCreateModal ? 'flex' : 'none'};">
                    <div class="modal-content" style="width: 700px; max-width: 90%; max-height: 90vh; overflow-y: auto;">
                        ...
                        <form id="createForm" action="${pageContext.request.contextPath}/product" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="action" value="create">
                            <input type="hidden" name="tab" value="${currentTab}">
                            
                            <div style="margin-bottom: 15px;">
                                <label style="font-weight:bold;">Product Type:</label><br>
                                <label><input type="radio" name="productType" value="Book" ${productCreate.type != 'Stationery' ? 'checked' : ''} onclick="toggleProductType('addPopup', 'Book')"> Book</label>
                                <label style="margin-left:20px;"><input type="radio" name="productType" value="Stationery" ${productCreate.type == 'Stationery' ? 'checked' : ''} onclick="toggleProductType('addPopup', 'Stationery')"> Stationery</label>
                            </div>

                            <div style="margin-bottom: 15px;">
                                <label>Name: *</label>
                                <input type="text" name="name" style="width:100%; padding:8px;" value="${productCreate.name}" required>
                            </div>
                            
                            <div style="display:flex; gap:15px; margin-bottom: 15px;">
                                <div style="flex:1;">
                                    <label>Price (đ): *</label>
                                    <input type="number" name="price" min="1" step="0.01" style="width:100%; padding:8px;" value="${productCreate.price}" required>
                                </div>
                                <div style="flex:1;">
                                    <label>Status:</label>
                                    <select name="status" style="width:100%; padding:8px;">
                                        <option value="1" ${productCreate.status == 1 ? 'selected' : ''}>Active</option>
                                        <option value="0" ${productCreate != null && productCreate.status == 0 ? 'selected' : ''}>Inactive</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div style="margin-bottom: 15px;">
                                <label>Image:</label>
                                <input type="file" name="imageFile" accept="image/*" style="width:100%; padding:8px;">
                            </div>

                            <div class="book-category" style="display: ${productCreate == null || productCreate.type != 'Stationery' ? 'block' : 'none'}; margin-bottom: 15px;">
                                <label>Category (Book): *</label>
                                <select name="bookCategoryId" style="width:100%; padding:8px;">
                                    <option value="">-- Select Category --</option>
                                    <c:forEach var="cat" items="${bookCategories}">
                                        <option value="${cat.categoryId}" ${productCreate.categoryId.categoryId == cat.categoryId ? 'selected' : ''}>${cat.categoryName}</option>
                                    </c:forEach>
                                </select>
                            </div>

                            <div class="stationery-category" style="display: ${productCreate.type == 'Stationery' ? 'block' : 'none'}; margin-bottom: 15px;">
                                <label>Category (Stationery): *</label>
                                <select name="stationeryCategoryId" style="width:100%; padding:8px;">
                                    <option value="">-- Select Category --</option>
                            ...
                            <div class="book-fields" style="display: ${productCreate == null || productCreate.type != 'Stationery' ? 'block' : 'none'}; background:#f9f9f9; padding:15px; margin-bottom:15px; border-radius:5px;">
                                <h4 style="margin-top:0;">Book Details</h4>
                                <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                    <div style="flex:1;">
                                        <label>ISBN: *</label>
                                        <input type="text" name="isbn" style="width:100%; padding:8px;" value="${productCreate.isbn}">
                                    </div>
                                    <div style="flex:1;">
                                        <label>Published Year:</label>
                                        <input type="number" name="publishedYear" min="0" max="2026" style="width:100%; padding:8px;" value="${productCreate.publishedYear}">
                                    </div>
                                </div>
                                <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                    <div style="flex:1;">
                                        <label>Publisher:</label>
                                        <input type="text" name="publisher" style="width:100%; padding:8px;" value="${productCreate.publisher}">
                                    </div>
                                    <div style="flex:1;">
                                        <label>Translator:</label>
                                        <input type="text" name="translator" style="width:100%; padding:8px;" value="${productCreate.translator}">
                                    </div>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <label>Genre: *</label>
                                    <select name="genreId" style="width:100%; padding:8px;">
                                ...
                                <div>
                                    ...
                                    <select name="authorIds" multiple style="width:100%; padding:8px; height:80px;">
                                    ...
                                    <small style="color:#666;">Hold Ctrl (Windows) or Cmd (Mac) to select multiple</small>
                                </div>
                            </div>
                            
                            <div class="stationery-fields" style="display: ${productCreate.type == 'Stationery' ? 'block' : 'none'}; background:#f9f9f9; padding:15px; margin-bottom:15px; border-radius:5px;">
                                <h4 style="margin-top:0;">Stationery Details</h4>
                                <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                    <div style="flex:1;">
                                        <label>Color:</label>
                                        <input type="text" name="color" style="width:100%; padding:8px;" value="${productCreate.color}">
                                    </div>
                                    <div style="flex:1;">
                                        <label>Material:</label>
                                        <input type="text" name="material" style="width:100%; padding:8px;" value="${productCreate.material}">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                <textarea name="description" rows="3" style="width:100%; padding:8px;">${productCreate.descriptionText}</textarea>
                            </div>

                            <div class="modal-footer">
                                <button type="button" class="btn-cancel" onclick="document.getElementById('addPopup').style.display='none'">Cancel</button>
                                <button type="submit" class="btn-save" style="background:#5cb85c; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">Save Product</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div id="editPopup" class="modal-overlay" style="display: flex;">
                        <div class="modal-content" style="width: 700px; max-width: 90%; max-height: 90vh; overflow-y: auto;">
                            ...
                            <form action="${pageContext.request.contextPath}/product" method="post" enctype="multipart/form-data">
                                ...
                                <div style="margin-bottom: 15px;">
                                    <label>Name: *</label>
                                    <input type="text" name="name" style="width:100%; padding:8px;" value="${productEdit.name}" required>
                                </div>
                                
                                <div style="display:flex; gap:15px; margin-bottom: 15px;">
                                    <div style="flex:1;">
                                        <label>Price (đ): *</label>
                                        <input type="number" name="price" min="1" step="0.01" style="width:100%; padding:8px;" value="${productEdit.price}" required>
                                    </div>
                                    <div style="flex:1;">
                                        <label>Status:</label>
                                        <select name="status" style="width:100%; padding:8px;">
                                ...
                                <div style="margin-bottom: 15px;">
                                    ...
                                    <select name="categoryId" style="width:100%; padding:8px;" required>
                                        <option value="">-- Select Category --</option>
                                        <c:forEach var="cat" items="${categories}">
                                            <option value="${cat.categoryId}" ${productEdit.categoryId.categoryId == cat.categoryId ? 'selected' : ''}>${cat.categoryName}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                                ...
                                <div style="margin-bottom: 15px;">
                                    ...
                                    <c:if test="${not empty productEdit.imageUrl}">
                                        <img src="${productEdit.imageUrl}" style="max-height: 100px; margin-bottom: 10px;"><br>
                                    </c:if>
                                ...
                                <c:if test="${productEdit.type == 'Book'}">
                                    <div style="background:#f9f9f9; padding:15px; margin-bottom:15px; border-radius:5px;">
                                        ...
                                        <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                            <div style="flex:1;">
                                                <label>ISBN: *</label>
                                                <input type="text" name="isbn" style="width:100%; padding:8px;" value="${productEdit.isbn}">
                                            </div>
                                            <div style="flex:1;">
                                                <label>Published Year:</label>
                                                <input type="number" name="publishedYear" min="0" max="2026" style="width:100%; padding:8px;" value="${productEdit.publishedYear}">
                                            </div>
                                        </div>
                                        <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                            <div style="flex:1;">
                                                <label>Publisher:</label>
                                                <input type="text" name="publisher" style="width:100%; padding:8px;" value="${productEdit.publisher}">
                                            </div>
                                            <div style="flex:1;">
                                                <label>Translator:</label>
                                                <input type="text" name="translator" style="width:100%; padding:8px;" value="${productEdit.translator}">
                                            </div>
                                        </div>
                                        <div style="margin-bottom: 10px;">
                                            <label>Genre: *</label>
                                            <select name="genreId" style="width:100%; padding:8px;">
                                                <option value="">-- Select Genre --</option>
                                                <c:forEach var="g" items="${genres}">
                                                    <option value="${g.genreId}" ${productEdit.genreId.genreId == g.genreId ? 'selected' : ''}>${g.genreName}</option>
                                                </c:forEach>
                                            </select>
                                        </div>
                                        <div>
                                ...
                                <c:if test="${productEdit.type == 'Stationery'}">
                                    <div style="background:#f9f9f9; padding:15px; margin-bottom:15px; border-radius:5px;">
                                        ...
                                        <div style="display:flex; gap:15px; margin-bottom: 10px;">
                                            <div style="flex:1;">
                                                <label>Color:</label>
                                                <input type="text" name="color" style="width:100%; padding:8px;" value="${productEdit.color}">
                                            </div>
                                            <div style="flex:1;">
                                                <label>Material:</label>
                                                <input type="text" name="material" style="width:100%; padding:8px;" value="${productEdit.material}">
                                            </div>
                                        </div>
                                    </div>
                                </c:if>
                                ...
                                <div style="margin-bottom: 15px;">
                                    <label>Description:</label>
                                    <textarea name="description" rows="3" style="width:100%; padding:8px;">${productEdit.descriptionText}</textarea>
                                </div>

                                <div class="modal-footer">
                                    <a href="${pageContext.request.contextPath}/product?tab=${currentTab}" class="btn-cancel" style="text-decoration:none;">Cancel</a>
                                    <button type="submit" class="btn-save" style="background:#337ab7; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">Update Product</button>
                                </div>
                            </form>
                        </div>
                    </div>
