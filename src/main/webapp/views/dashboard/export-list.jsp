<%-- Document : export-list Created on : Mar 3, 2026, 5:05:34 PM Export : LECOO --%>
                                                ...
                                                <%-- Detail Import --%>
                                                    <div class="action-buttons">
                                                        <button type="button" class="btn-action btn-detail"
                                                            onclick="openDetailPopup(${e.orderId}, 1)" title="Detail">
                                                            <i class="bi bi-eye"></i>
                            ...
                            <%-- Previous button --%>
                                <c:if test="${currentPage > 1}">
                                    <a class="page-btn ${currentPage == 1 ? 'disabled' : ''}"
                                        href="inventory?view=export-list&page=${currentPage - 1}&fromDate=${fromDate}&toDate=${toDate}">&laquo;</a>
                                </c:if>
                                <%-- If the total <=5, display all pages. --%>
                                    <c:if test="${totalPages <= maxNote}">
                                        <c:forEach begin="1" end="${totalPages}" var="i">
                                            <a class="page-btn ${currentPage == i ? 'active' : ''}"
                                                href="inventory?view=export-list&page=${i}&fromDate=${fromDate}&toDate=${toDate}">${i}</a>
                                        </c:forEach>
                                    </c:if>
                                    <%-- If the total> 5 --%>
                                        <c:if test="${totalPages > maxNote}">
                                            <%-- Page 1 always appears --%>
                                                <a class="page-btn ${currentPage == 1 ? 'active' : ''}"
                                                    href="inventory?view=export-list&page=1&fromDate=${fromDate}&toDate=${toDate}">1</a>
                                                        ...
                                                        <%-- The final ellipsis --%>
                                                            ...
                                                            <%-- The last page always appears --%>
                                                                <a class="page-btn ${currentPage == totalPages ? 'active' : ''}"
                                                                    href="inventory?view=export-list&page=${totalPages}&fromDate=${fromDate}&toDate=${toDate}">
                                                                    ${totalPages}
                                                                </a>
                                        </c:if>
                                        <%-- Next button --%>
                                            <c:if test="${currentPage < totalPages}">
                                                <a class="page-btn ${currentPage == totalPages ? 'disabled' : ''}"
                                                    href="inventory?view=export-list&page=${currentPage + 1}&fromDate=${fromDate}&toDate=${toDate}">&raquo;</a>
                                            </c:if>
                        </ul>
                    </nav>
                </div>
