        ...
        <nav class="d-flex justify-content-center">
            <ul class="pagination">
                ...
                <c:if test="${currentPage > 1}">
                    <a class="page-btn ${currentPage == 1 ? 'disabled' : ''}" href="inventory?view=export-list&page=${currentPage - 1}">&laquo;</a>
                </c:if>
                <%-- If the total <= 5, display all pages. --%>
                <c:if test="${totalPages <= 5}">
                    <c:forEach begin="1" end="${totalPages}" var="i">
                        <a class="page-btn ${currentPage == i ? 'active' : ''}" href="inventory?view=export-list&page=${i}">${i}</a>
                    </c:forEach>
                </c:if>
                <%-- If the total > 5 --%>
                <c:if test="${totalPages > 5}">
                    <%-- Page 1 always appears --%>                   
                    <a class="page-btn ${currentPage == 1 ? 'active' : ''}"href="inventory?view=export-list&page=1">1</a>
                    ...
                    <a class="page-btn ${currentPage == totalPages ? 'active' : ''}" href="inventory?view=export-list&page=${totalPages}">
                        ${totalPages}
                    </a>
                </c:if>
                <%-- Next button --%>
                <c:if test="${currentPage < totalPages}">
                    <a class="page-btn ${currentPage == totalPages ? 'disabled' : ''}" href="inventory?view=export-list&page=${currentPage + 1}">&raquo;</a>
                </c:if>
            </ul>
        </nav>
    </div>
